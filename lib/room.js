...
async function createRoom({ io, theme, numOfPlayers, gridSize, hostClientId }) {
  ...
  const value = JSON.stringify({ theme, numOfPlayers, gridSize, hostClientId });

  await redisClient.setEx(roomKey, EXPIRY_DURATION, value);
  return room;
}
...
async function loadRoom(io, roomId) {
  ...
  const roomConfigKey = `room:${roomId}`;
  let roomConfig = await redisClient.get(roomConfigKey);
  if (roomConfig) {
    const { theme, numOfPlayers, gridSize, hostClientId } =
      JSON.parse(roomConfig);
    const room = new Room({
      io,
      theme,
      numOfPlayers,
